...
public interface PinRepository extends JpaRepository <Pin, Long> {
    ...
    List<Pin> findAllBySongAndCreator(Song song, Member member);
    List<Pin> findAllByCreatorOrderByListenedDateDesc(Member member);
    List<Pin> findAllByCreatorAndVisibilityOrderByListenedDateDesc(Member member, Visibility visibility);
    ...
    @Query("SELECT p FROM Pin p WHERE p.creator = :member AND YEAR(p.listenedDate) = :year AND MONTH(p.listenedDate) = :month")
    List<Pin> findAllByCreatorAndDate(@Param("member") Member member, @Param("year") int year, @Param("month") int month);
